...
def test_repo_source_tree_under_limit() -> None:
    ...
    # Use the module's SCAN default so the safety net matches what CI enforces
    # (src, tests AND scripts) rather than a narrower hand-picked subset.
    over = mod.scan_dirs(root, mod.SCAN)
