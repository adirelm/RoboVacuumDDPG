...
> **Status: in progress — training not yet run.** The simulator (`VacuumEnv`),
> the DDPG agent (Actor / Critic / Polyak soft-update / Gaussian noise), the SDK,
> and the figure/report scaffolding are the build targets of this assignment.
> **All numeric results below are marked `PENDING` until a seeded training run
> completes** — no result numbers are invented here. Quality gates target: ruff
> clean · ≥85% coverage (`fail_under=85`) · every `.py` ≤150 LOC · `uv` only.
...
The three brief-mandated artifacts. All numbers are **`PENDING`** — figures are
regenerated by the scripts above once a seeded training run completes.
...
smooth continuous coverage (optional short MP4/GIF animation). Embed pending
training:

> ![Trajectory over HouseExpo map — pending training](results/figures/trajectory.png)
> *PENDING — generated by `uv run python scripts/render_trajectory.py` after `train.py`.*

### 3.2 Metric graphs (the two required figures)

**Learning curve** — cumulative reward vs. episode, mean ± CI over the
seeds `[42, 7, 123, 314, 271]`:

> ![Learning curve — pending training](results/figures/learning_curve.png)
> *PENDING — `uv run python scripts/render_learning_curve.py`.*

**Critic loss** — critic loss vs. training step:

> ![Critic loss — pending training](results/figures/critic_loss.png)
> *PENDING — `uv run python scripts/render_critic_loss.py`.*
