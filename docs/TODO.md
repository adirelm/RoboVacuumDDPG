...
4. **Evidence pointer** recorded in the commit message body — file path /
   ...
   acceptable. The evidence pointers are indexed by the PRD §5.1 trace
   table (the planned separate TRACE.md was folded into it).
...
| T01-02 | Implement `src/env/raycast.py` — ray–segment intersection → lidar distances (per-segment math loop, spec §10) | Implementing raycaster | 1 | +130 | SIM-raycast | `test_raycast_hits_known_wall_at_expected_distance` (hand-computed geometry: axis-aligned wall, 45° ray) + `test_ray_misses_returns_ray_max` |
...
| T02-07 | Implement `src/cost/meter.py` — `RuntimeMeter` wall-clock + step/episode cost accounting (spec §4/§11) feeding `docs/COST_ANALYSIS.md` | Implementing cost meter | 2 | +90 | COST-meter | `tests/unit/test_meter.py` (runtime + counters) |
...
| T03-03 | Implement `scripts/train_multi_seed.py` (or `RoboVacuumSDK.train` driver) — train across **5 seeds {42, 7, 123, 314, 271}** (config `training.seeds`, `episodes=500`) on the train maps; persist per-seed reward arrays + critic-loss arrays | Running multi-seed training | 3 | +120 | TRAIN-multiseed | Five per-seed result artifacts exist; each carries reward-vs-episode + critic-loss-vs-step arrays |
...
| T04-03 | Author `docs/COST_ANALYSIS.md` — concrete artifact-corpus headline + chars/bytes appendix, AI-tooling + training-runtime cost table, **cost envelope** (architect-decided spend cap vs running total); `src/cost/meter.py` covers runtime | Authoring COST_ANALYSIS.md | 4 | +180 docs | DOC-cost | Headline is the measured corpus size; appendix has chars + bytes; cost envelope section names cap + actual spend |
...
| T04-10 | Invite `@rmisegal` as read-only collaborator on `github.com/adirelm/RoboVacuumDDPG`; record invitation ID in `docs/SUBMISSION.md` (spec §2) | Inviting @rmisegal as read-only collaborator | 4 | +0 | (submission) | Invitation sent with **Read** role to GitHub handle `@rmisegal`; invitation ID recorded |
